...
function loadPlayersPage(){
	...
	for(var i = 0 ; i < players.length; i++){
		...
		option.addEventListener('click', switchPlayers(i));
...
function loadStatsPage(){
	...
	var celv_val;
	 ...
	 for( y = 2; y < table.rows.length; y++){
		 if( table.rows[y].cells[x].innerHTML > table.rows[y].cells[x+1].innerHTML){
			 table.rows[y].cells[x+2].innerHTML = "Winner";
			 w++;
		 }else{
			 table.rows[y].cells[x+2].innerHTML = "Losser";
			 l++;
		 }
